...
p1 <- ggplot(Book6, aes(x=chapter, y=n)) +
      ...
      geom_smooth(se=FALSE, size = 0.3, linetype = 4, color = "green")
...
(7) Plotting different colors for the most positive and negative words in Book6
```{r}
library(reshape2)
library(wordcloud)
...
(9) count the most common bigrams and delete the stop_words
```{r}
library(tidyr)

#split bigram into two columns, which are word1 and word2
bigrams_separated <- Book6bi %>%
...
realresult <- merge(newdata, Book6, by="chapter") %>%
  mutate(n= (n.x + n.y))

x <- realresult %>%
  select(chapter, n.y, n)

p2 <- ggplot(x, aes(x= chapter)) + 
  geom_line(aes(y = n.y, colour = "red")) + 
