...
*********Interpretation for lm? or loess? ADD LEGEND *********
...
ggplot(anticipation, aes(x=chapter, y=n)) +
...
2. What is the relative contribution of each word to the sentiment of the book "Chamber of Secrets"? 
```{r}
book2 %>%
  # Count by word
  count(word, sort = TRUE) %>%
...
sentiment_by_book %>%
    ...
    count(book, sentiment, total_words) %>%
    ungroup() %>%
    mutate(percent = n / total_words) %>%
    # Set up the plot with aes()
    ggplot(aes(book, percent, color=sentiment)) +
    geom_line(size = 1.5) +
